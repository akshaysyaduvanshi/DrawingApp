...
class LineInputValidatorTest {
  ...
  public void shouldThrowInvalidArgumentsException() {
    IllegalArgumentException exception1 = assertThrows(IllegalArgumentException.class, () -> {
      new LineInputValidator().validate(canvas, 1);
    });

    assertEquals(exception1.getMessage(), "Invalid input to draw a line");

    IllegalArgumentException exception2 = assertThrows(IllegalArgumentException.class, () -> {
      new LineInputValidator().validate(canvas, 100,10,12,1);
    });
  ...
  public void shouldReturnTrueForValidInputs() {
    assertTrue(new LineInputValidator().validate(canvas,1,2,2,1));
  }
}